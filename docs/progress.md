...
- **Current sprint:** **Post-M1 buffer — View tracking + popularity v2 (2026-05-06)** ✅ — fifth and final buffer feature, completing the owner's tier-1 ask ("الأقسام اللي يزورها العميل باستمرار"). New append-only `ProductView` + `CategoryView` tables logged from the SSR render of `/products/[slug]` and `/categories/[slug]`. Bot UA filter at the recording layer keeps Googlebot/Bingbot/social-card fetchers/curl out of the score. Popularity formula expanded: `5 × order_qty + product_view_count + category_boost` where the category boost now blends category orders (5×) with category product-views and direct category-page views. 90-day retention via new `cleanup-expired-views` cron at 03:15 EET (just before the 03:30 recompute). SQL string lifted into `lib/catalog/popularity.ts::RECOMPUTE_POPULARITY_SQL` so post-push.ts and the worker share one source of truth.
- **Last updated:** 2026-05-06 — view tracking shipped on `claude/view-tracking` branch.
