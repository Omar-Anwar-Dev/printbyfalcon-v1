...
- **Current sprint:** **Post-M1 buffer — Home page polish round (2026-05-06)** ✅ — staging review of PR #76 surfaced four owner asks: (1) drop the "أصلية ومتوافقة" qualifier from the hero h1 (just `طابعات وأحبار` now), (2) bump the hero subtitle one size up so the identity paragraph reads stronger, (3) rename the third value-prop from "شحن لكل المحافظات / Delivery to all 27 governorates" to a non-numeric "شحن سريع / Fast shipping" (owner doesn't want to commit to a hard governorate count in the copy), (4) remove the entire "تصفح الفئات / Shop by category" image-tile rail — it was duplicating the navigation bar's category dropdowns and the home page felt cleaner without it. Removing the rail also dropped the unused `categoryImageUrl` import + `TopCategory` type + `prisma.category.findMany` call from the home page.
- **Last updated:** 2026-05-06 — polish shipped on `claude/home-copy-polish` branch.
