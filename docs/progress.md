...
- **Current sprint:** **Post-M1 buffer — JSON paste completeness (2026-05-06)** ✅ — small follow-up to PR #73's JSON quick-fill panel after PR #75 added `Product.printerModelId`. The paste blob now also accepts `printerModel` (free-form name like "HP LaserJet Pro M404dn", resolves through brand+modelName) and `printerModelId` (direct cuid), with `null` to clear an existing link. New `getPrinterModelResolveData()` exposes the resolution data; the synthetic `nameAr/nameEn` fields use `${brand} ${modelName}` so the existing `resolveOption` substring matcher works without changes. Sample JSON + ChatGPT prompt updated to mention the new field as optional (only meaningful for products that ARE printers).
- **Last updated:** 2026-05-06 — JSON paste completeness shipped on `claude/json-fill-completeness` branch.
