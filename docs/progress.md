...
- **Current sprint:** **Post-M1 buffer — Default sort by popularity (2026-05-06)** ✅ — owner-requested fourth buffer feature: stop ranking listings by `createdAt DESC`. New `Product.popularityScore` column (default 0, indexed compound with `createdAt DESC`) populated nightly by the `recompute-popularity` worker job from order-item quantities over the last 90 days plus a small category boost (0–10 points) so new SKUs in a hot category outrank new SKUs in a quiet one. New `recommended` sort option becomes the default on `/products` and `/categories/[slug]`; on `/search`, `relevance` stays the default for queried searches and `recommended` is the default when no query. Cancelled / returned orders are excluded from the score. Initial backfill runs from `scripts/post-push.ts` on every deploy so prod gets fresh scores immediately rather than waiting up to 24h for the cron's first tick. Builds on PR #75 (compatibility), PR #76 + PR #77 (home copy + polish), PR #78 (search dropdown contrast) — the previous M1→M2 buffer changes already on `main`.
- **Last updated:** 2026-05-06 — popularity sort shipped on `claude/popularity-sort` branch.
