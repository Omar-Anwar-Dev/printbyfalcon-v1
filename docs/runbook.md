...
| 2026-04-22 | staging | `a64f4d8` | **Sprints 7 + 8 + 9 + 10 combined bundle.** B2B portal + Submit-for-Review + bulk-order + reorder (S7+S8) + COD + 5-zone shipping + promo codes + VAT + 8-card settings hub (S9) + Admin Completeness: role audit, `/admin/users` with 48h invite flow, `/admin/customers`, Return policy + per-product `returnable` + override + auto stock-release, dashboard widgets + SVG trend chart, WhatsApp "Chat with us" bridge, pre-confirmation COD line edit, order CSV export, brand logo on invoice (S10). Deploy clean; all staging smoke tests green after setting `supportWhatsapp` via admin UI. |
| 2026-04-22 | prod | `a64f4d8` | **Sprints 7 + 8 + 9 + 10 combined bundle.** Manual-SSH deploy (runbook §4.3). All 7 containers healthy post-deploy; `🚀 DB is now in sync with Prisma schema. Done in 323ms` (4 new columns: `Product.returnable`, `Return.policyOverride/overrideReason/stockReleasedAt`); FTS bootstrap rewrote 200 product search vectors; `returns.policy` Setting auto-seeded via post-push (enabled / 14-day / no min-order / all-roles override). 16 min of clean worker heartbeats post-deploy — no errors, no exceptions. `supportWhatsapp` configured post-deploy via `/admin/settings/store`. No rollback needed. |
